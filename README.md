...
드론 항공 영상에서 사람을 탐지하려고 만든 YOLOv8 모델입니다.
작은 객체가 많고 위에서 내려다보는 시점이라 일반 객체 탐지보다 난이도가 높습니다.

> ⚠️ **먼저 읽어 주세요 — 아래 수치는 "사람 탐지" 성능이 아닙니다.**
> 목표는 사람 인식이었지만, 실제 학습 라벨은 사람뿐 아니라 **차량을 포함한 모든 객체를 하나의 클래스(`item`)로 합친 것**이었습니다.
> 그래서 아래 성능 수치는 모두 **클래스를 구분하지 않은 객체 탐지** 기준입니다. 자세한 내용은 [사후 점검](#사후-점검--라벨이-지표의-뜻을-바꾼다) 참고.
...
1. `runs/train3/labels.jpg` — 학습 인스턴스가 `item` 한 클래스에 **343,201개**입니다. 원본 라벨을 직접 세어 보면 다음과 같습니다.

   | 구분 | 객체 수 |
   |---|---|
   | `pedestrian` (class 0) | 79,337 |
   | `people` (class 1) | 27,059 |
   | **사람 합계** | **106,396** |
   | 나머지 8개 클래스 (car 144,867 등) | 236,809 |
   | **전체 합계** | **343,205** |

   사람만 남겼다면 약 10.6만 개여야 하는데 34.3만 개가 들어갔습니다. 전체 합계와 4개 차이는 크기가 0인 박스가 걸러진 정도로 보입니다.
